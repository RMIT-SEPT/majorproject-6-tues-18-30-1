...
const ColumnCell = props => {
  ...
  // If the data for that hour is an array, this means there are two bookings to be rendered
    ...
    <table className="table--nested">
      ...
      <tr className="table__row--no-border table__row--border-bottom">
          <td
        onClick={() => props.onShowBooking(firstBookingData)} className={`table__cell--${firstBookingData.businessId
              .replace(/ /g, '-')
            .toLowerCase()} table__cell--subcell`
            }
          >
            {firstBookingData.businessId}
          </td>
        </tr>
        <tr className="table__row--no-border">
          <td
        onClick={() => props.onShowBooking(secondBookingData)} className={`table__cell--${secondBookingData.businessId
              .replace(/ /g, '-')
            .toLowerCase()} table__cell--subcell`
            }
          >
            {secondBookingData.businessId}
          </td>
        </tr>
      </tbody>
    </table>
  ...
  // If there is a booking object, but only for the first half of the hour, return a nested table to split the table data for that cell into two rows.
    ...
    columnData =
        <table className="table--nested">
          ...
          <tr className="table__row--no-border table__row--border-bottom">
              <td
            onClick={() => props.onShowBooking(bookingData)} className={`table__cell--${bookingData.businessId
                  .replace(/ /g, '-')
                .toLowerCase()} table__cell--subcell`
                }
              >
    ...
    columnData =
        <table className="table--nested">
          ...
          <tr className="table__row--no-border table__row--border-bottom">
            ...
            <tr className="table__row--no-border">
          <td onClick={() => props.onShowBooking(bookingData)} className={`table__cell--${bookingData.businessId
                  .replace(/ /g, '-')
                  .toLowerCase()} table__cell--subcell`
                }
              >
  ...
  // If there is a booking object for the full hour, return a single <td> cell
    ...
    columnData =
      ...
      className={`table__cell--${bookingData.businessId
          .replace(/ /g, '-')
          .toLowerCase()}`
        }>
          {bookingData.businessId}
